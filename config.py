...
class Config(BaseModel):
    ...
    maimaidxpath: str
    botName: str = list(driver.config.nickname)[0] if driver.config.nickname else 'wmc'
...
if maiconfig.maimaidxpath:
    ...
    raise ValueError('`nonebot-plugin-maimaidx-extra` 插件未检测到静态文件夹 `static`，请根据 README 配置页说明进行下载静态文件')
